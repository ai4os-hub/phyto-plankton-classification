...
def email_scheduler():
    global last_summary_date, last_afternoon_summary_sent_day
    while True:
        ...
        if last_summary_date != current_date:
            send_daily_summary_email(
                last_summary_date.strftime('%Y-%m-%d') if last_summary_date else (
                            current_date - datetime.timedelta(days=1)).strftime('%Y-%m-%d'),
                daily_tar_reports
            )
            daily_tar_reports.clear()
            last_summary_date = current_date

        # 15:45 summary
        if (now.time() >= datetime.time(10, 1) and
...
def send_daily_summary_email(summary_date, report_data):
    ...
    # === Count current files ===
    preview_root = Path("Y:/plankton-imager-10/not_processed/previews")
    current_counts = {
    ...
    required_outputs = {
        ...
        "_predictions_relative.json": ("Predictions (JSON)", 4),  # 4 hours per file
...
# === SETUP ===
source_dir = Path("Y:/plankton-imager-10/processed")
work_dir = Path("D:/USERS/jonas.mortelmans/PI10_tempUntarred")
quarantine_dir = Path("Y:/plankton-imager-10/processed/quarantine")
...
def outputs_exist_for_tar(tar_file):
    ...
    preview_dir = Path(r"Y:/plankton-imager-10/not_processed/previews") / f"{stem}_preview"
...
def get_new_tar_files(source_dir):
    ...
    preview_dir = Path(r"Y:/plankton-imager-10/not_processed/previews")
...
def extract_exif_metadata(tif_paths, tar_source, batch_size=200):
    ...
    for batch_idx in tqdm(range(n_batches), desc="ExifTool Batches", unit="batch"):
        ...
        try:
            ...
            print(f"❌ EXIF error in batch {batch_idx}: {e}")
    ...
    print(f"✅ EXIF extraction complete: {len(df)} of {len(tif_paths)} files.")
    return df



def write_exif_csvs(df, tar_name, output_dir, backup_dir):
    print("✅ Saving EXIF metadata CSVs...")
    df["tif_name"] = df["SourceFile"].apply(lambda x: os.path.basename(x))
...
def classify_and_extract_regions(tar_file, extract_dir):
    ...
    print(f"📊 Processing {len(FILEPATHS)} TIFF files in {base_name}...")
    ...
    print(f"✅ Saved predictions JSON: {json_path.name}")
...
def generate_topspecies_csv(json_path):
    ...
    print(f"✅ Saved top species CSV: {output_path}")
...
def process_tar(tar_file):
    ...
    try:
        ...
        # === Step 5–9: Processing steps ===
        if not bg_path.exists():
            copy_background_tif(extract_dir, bg_path)
            status_log.append("Background.tif copied")
        else:
...
#     test_summary_date = datetime.datetime.now().strftime('%Y-%m-%d')
#     test_report_data = [{"tar_name": "TEST_TAR_123", "status_log": ["..."]}]
#     send_daily_summary_email(test_summary_date, test_report_data)

# === LIBRARIES ===
import os
# Suppress TensorFlow logs: INFO=0, WARNING=1, ERROR=2
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3'  # only ERROR
# Suppress XLA / CUDA backend logs
os.environ['XLA_FLAGS'] = '--xla_gpu_strict_conv_algorithm_picker=false --xla_hlo_profile=false --xla_gpu_force_compilation_parallelism=1'

# Optional: prevent absl logging clutter
...
log_dir = Path("path/pltankton-imager-10/GPU_ENVIRONMENT/logs")
...
preview_path = Path("path/pltankton-imager-10/not_processed/previews")
exiftool_path= Path("path/pltankton-imager-10/GPU_ENVIRONMENT/exiftool-13.31_64/exiftool.exe")

exiftool_path = "exiftool"

# === DIRECTORIES ===

source_dir= Path("path/pltankton-imager-10/processed/priority - copies can be deleted")
# source_dir= Path("path/pltankton-imager-10/wout")
work_dir= Path("path/pltankton-imager-10/GPU_ENVIRONMENT/PI10_tempUntarred")
gpu_env= Path("path/pltankton-imager-10/GPU_ENVIRONMENT")
...
def send_daily_summary_email(summary_date, report_data):
    ...
    # === Count current files ===
    preview_root = Path(preview_path)
    current_counts = {
...
def outputs_exist_for_tar(tar_file):
    ...
    preview_dir = Path(preview_path) / f"{stem}_preview"
...
def get_new_tar_files(source_dir):
    ...
    for tar in all_tar:
        ...
        preview_dir = Path("path/pltankton-imager-10/not_processed/previews")
            ...
            if len(df) < 10:  # should be at least 10 rows
                ...
                try:
                    if output_file.exists():
                        ...
                        #print(f"       ⚠️ Removed hitsmisses.txt for {tar_file.name} after quarantine")
                except Exception as e:
...
def create_preview_images(extract_dir, preview_dir, tar_name, n=20):
    start = timer()
...
def extract_exif_metadata(tif_paths, tar_source, batch_size=200):
    ...
    for batch_idx in range(n_batches):
        ...
        args = [
            exiftool_path,  # now points to Linux binary
            '-GPSLatitude',
            '-GPSLongitude',
            '-FileModifyDate',
            '-DateTimeOriginal',
...
def generate_topspecies_csv(json_path):
    print(f"⚙ Generating top species CSV")

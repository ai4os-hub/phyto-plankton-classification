...
class MLflowReceiver(AnalyticsReceiver):
    ...
    def initialize(self, fl_ctx: FLContext):
        ...
        sites = {site for site in sites if site.name == site_name}
        
        self._init_buffer(sites)
        self.run_ids = self.mlflow_setup(art_full_path, experiment_name, experiment_tags, sites)

        fl_ctx.set_prop('RUN_IDS', self.run_ids, sticky=True)
